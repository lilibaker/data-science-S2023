...
df_normalized %>%
  ...
  geom_hline(yintercept = 9974.675) +
...
df_normalized %>%
  ...
  ggplot(  ) +
  geom_line(aes(date, cases_per100k)) +
  geom_line(aes(date, deaths_per100k)) + 
  scale_y_log10(labels = scales::label_number_si()) +
  theme_minimal() +
  labs(
    x = "Date",
...
df_normalized %>%
  ...
  ggplot() +
  geom_point(aes(cases_per100k, deaths_per100k)) +
  geom_point(
    data = . %>% filter(county %in% c("Miami-Dade", "Broward", "Palm Beach")),
    aes(cases_per100k, deaths_per100k, color = county)
  ) +
  geom_hline(yintercept = 9974.675) +
  scale_y_log10(labels = scales::label_number_si()) +
  scale_color_discrete(name = "County") +
...
df_normalized %>%
  ...
  ggplot(aes(state, max(cases_per100k))) +
  geom_point() +
  # geom_point(
  #   data = . %>% filter(state == ),
  #   aes(color = county)
