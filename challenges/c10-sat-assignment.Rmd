...
- What kinds of information *do we not have* about these students?
  - The classes they are taking that result in their gpa; individual grading systems at the schools that affect the gpas relative to one another; and whether the students had sat prep courses.
- Based on these missing variables, what possible effects could be present in the data that we would have *no way of detecting*?
  - The factors could cause students with high SAT scores to exhibit lower GPAs as their classes may be harder or they may have had courses that focused on increasing SAT scores. Additionally, the GPAs may exhibit large differences if the students in certain classes are grader harsher than others.
...
- What relationship do `univ_GPA` and `both_SAT` exhibit?
  - As the university gpa increases, there tends to be an increase in the composite sat score. However, there is quite a bit of overlap in sat values in the 3.0-3.5 gpa range, which suggests that the relationship is more distinct at the extreme ends of the gpa spectrum.
- What relationship do `univ_GPA` and `high_GPA` exhibit?
  - Generally, as the high school gpa increases, so does the university gpa. However, there is also a significant portion of the data that exhibits a higher university gpa in the lower end of the high school gpa section.
...
- To what extent does `both_SAT` look like a normal distribution?
  - There is a congregation of values around 1150, but given the large amount of data in the higher end, there doesn't appear to be a normal distribution.
- To what extent does `high_GPA` look like a normal distribution?
  - It does not look like a normal distribution -- there is a fairly wide spread distribution of the data and there are large spikes in concentration in various sections.
- To what extent does `univ_GPA` look like a normal distribution?
  - There seems to be a normal-like section of the graph from 3.0 up, but there is still a significant portion of the data in the lower section.
...
- Which correlations are significantly nonzero?
  - Both of the correlations are significantly nonzero as they are greater than 0.5.
...
- How do the results here compare with the visual you created in q2?
  - The correlation between `high_GPA` and `univ_GPA` is seen in the visual in q2 as when `high_GPA` increases, `univ_GPA` also tended to increase. In terms of `high_GPA` and `both_SAT`, the weaker correlation is evident in the wide range of SAT values at a given GPA, as well as with the fact that when the GPA increased, the SAT did not necessarily increase, and vice versa.
- Based on these results, what can we say about the predictive capabilities of both `high_GPA` and `both_SAT` to predict `univ_GPA`?
  - Since the correlation between `high_GPA` and `univ_GPA` is higher than that of `both_SAT` and `univ_GPA`, it can be concluded that `high_GPA` has much higher predictive capabilities in the prediction of `univ_GPA`.
...
- How does your estimate from q5 compare with your estimate from q4?
  - The estimate is pretty similar to q4, with the correlation being just slight greater.
...
- What is the confidence interval on the coefficient of `both_SAT`? Is this coefficient significantly different from zero?
  - The confidence interval is between about 0.00198 and 0.00349, which is significantly different from zero as the entire interval is greater than zero.
- By itself, how well does `both_SAT` predict `univ_GPA`?
  - Not very well as while the coefficient and its confidence interval are greater than zero, they are still pretty close to zero, demonstrating that `both_SAT` does not have a strong effect in `univ_GPA`.
...
- What is the confidence interval on the coefficient of `both_SAT` when including `high_GPA` as a predictor?? Is this coefficient significantly different from zero?
  - The confidence interval is about -0.00022 to 0.0018, which is not significantly different from zero as it includes zero.
- How do the hypothesis test results compare with the results in q6?
  - From the coefficients and the confidence intervals, it is evident that `high_GPA` has a greater effect in `univ_GPA` than `both_SAT`.
...
- Between `both_SAT` and `high_GPA`, which single variable would you choose to predict `univ_GPA`? Why?
  - `high_GPA` as it has been shown to have a greater correlation and the linear model demonstrated that the `high_GPA` had a much greater effect on the `univ_GPA`.
- Is `both_SAT` an effective predictor of `univ_GPA`? What specific pieces of evidence do you have in favor of `both_SAT` being effective? What specific pieces of evidence do you have against?
  - No, `both_SAT` is not an effective predictor. While `both_SAT` and `univ_GPA` exhibited a positive correlation, the results of the linear model demonstrate that the effect of `both_SAT` on `univ_GPA` are not as strong as those of `high_GPA`.
