...
fit_lower <- diamonds_train %>% 
  filter(carat <= 1) %>% 
  lm(formula = price ~ cut)
fit_upper <- diamonds_train %>% 
  filter(carat > 1) %>% 
  lm(formula = price ~ cut)
...
the lower carat should have decreasing price with increasing cut, while the higher carat should be the opposite
...
fit_carat_lower <- diamonds_train %>% 
  filter(carat <= 1) %>% 
  lm(formula = carat ~ cut)
fit_carat_upper <- diamonds_train %>% 
  filter(carat > 1) %>% 
  lm(formula = carat ~ cut)
...
lower: carat decreases as cut increases
upper: carat tends to be level as cut changes
- How might this trend relate to the `price ~ cut` behavior we saw above?
lower: as the carat decreases with the increase in cut, so too does the price
upper: since the carat does not really decrease as the cut increases, then the price increases as the cut increases
