...
```
The second one uses a pipe which means that you dont have to enter the dataframe as an argument, but they both produce the same result.
...
```
the price is summed across color and clarity; the previous version summed across the whole dataframe
...
- Comment on the relative values of `cov_depth` and `cov_diff`.
  cov_diff is much larger than cov_depth
- Why is `cov_diff` so large?
  since there is such a small difference between depth and computed depth, cov_diff is so large
...
- it is surprising that the ideal cut is not the most expensive
- the 80-110% for cov suggests that other variables have a greater effect on the price
